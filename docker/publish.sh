...
docker push "$BASE_URL"/create_bsim_db:1.0.0
docker push "$BASE_URL"/create_bsim_db:latest

docker push "$BASE_URL"/elastic_bsim:1.0.0
docker push "$BASE_URL"/elastic_bsim:latest

docker push "$BASE_URL"/ghidra-bsim-postgres:1.0.0
...
docker push "$BASE_URL"/sighthouse-pipeline:1.0.0
docker push "$BASE_URL"/sighthouse-pipeline:latest

docker push "$BASE_URL"/sighthouse:1.0.0
docker push "$BASE_URL"/sighthouse:latest

docker push "$BASE_URL"/sighthouse-frontend:1.0.0
